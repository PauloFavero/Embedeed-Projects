...
void lcd_initLcd(void) {
	...
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_RS_DIR<<LCD_RS_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_ENABLE_DIR<<LCD_ENABLE_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB0_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB1_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB2_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB3_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB4_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB5_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB6_PIN);
	GPIOC_PDDR |= GPIO_PDDR_PDD(LCD_DATA_DIR<<LCD_DATA_DB7_PIN);
...
void lcd_write2Lcd(unsigned char ucBuffer, unsigned char cDataType) {
	...
	if (LCD_RS_CMD == cDataType) {
		/* will send a command */
		GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_RS_PIN);
		GPIOC_PSOR |= GPIO_PSOR_PTSO(LCD_RS_CMD<<LCD_RS_PIN);
	}
	...
	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB0_PIN);
	...
	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB0_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 0u)) >> 0u)<<LCD_DATA_DB0_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB1_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 1u)) >> 1u)<<LCD_DATA_DB1_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB2_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 2u)) >> 2u)<<LCD_DATA_DB2_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB3_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 3u)) >> 3u)<<LCD_DATA_DB3_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB4_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 4u)) >> 4u)<<LCD_DATA_DB4_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB5_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 5u)) >> 5u)<<LCD_DATA_DB5_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB6_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 6u)) >> 6u)<<LCD_DATA_DB6_PIN);

	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_DATA_DB7_PIN);
	GPIOC_PSOR |= GPIO_PSOR_PTSO(
			((ucBuffer & (1u << 7u)) >> 7u)<<LCD_DATA_DB7_PIN);
	...
	GPIOC_PCOR |= GPIO_PCOR_PTCO(1u<<LCD_ENABLE_PIN);
...
void lcd_dummyText(void) {
	...
	// send string
	lcd_writeString("108318 Guilherme");

	// set the cursor line 1, column 0
	lcd_setCursor(1, 0);
	lcd_writeString("147601 Paulo");
}
